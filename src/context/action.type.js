// App main reducer action

export const CLEAR_APP_STATE = 'CLEAR_APP_STATE';
export const IS_AUTHTHENTICATED = 'IS_AUTHTHENTICATED';
export const IS_EMAIL_VERIFIED = 'IS_EMAIL_VERIFIED';
export const IS_SIGNIN = 'IS_SIGNIN';
export const IS_POST_NOT_FOUND = 'IS_POST_NOT_FOUND';
export const REMOVE_MOVE_TO_BIN_POST = 'REMOVE_MOVE_TO_BIN_POST';
export const SET_EDIT_POST_DATA = 'SET_EDIT_POST_DATA';
export const SET_IS_LOADING = 'SET_IS_LOADING';
export const SET_LAST_USER_QUEARY_DOC = 'SET_LAST_USER_QUEARY_DOC';
export const SET_PUBLIC_POST = 'SET_PUBLIC_POST';
export const SET_USER = 'SET_USER';
export const SET_USER_BIN_POST = 'SET_USER_BIN_POST';
export const SET_USER_POST = 'SET_USER_POST';
export const SET_VIEW_POST_DATA = 'SET_VIEW_POST_DATA';
export const UPDATE_PUBLIC_POST = 'UPDATE_PUBLIC_POST';
export const UPDATE_USER_POST = 'UPDATE_USER_POST';

// Add post reducer action
export const ADD_POST_IMAGE = 'ADD_POST_IMAGE';
export const CLEAR_POST_STATE = 'CLEAR_POST_STATE';
export const SET_EDIT_POST_STATE = 'SET_EDIT_POST_STATE';
export const SET_IS_PRIVATE = 'SET_IS_PRIVATE';
export const UPDATE_EDITOR_STATE = 'UPDATE_EDITOR_STATE';
export const UPDATE_POST_ID = 'UPDATE_POST_ID';
export const UPDATE_POST_CATEGORY = 'UPDATE_POST_CATEGORY';
export const UPDATE_POST_SAMPLE = 'UPDATE_POST_SAMPLE';
export const UPDATE_POST_TITLE = 'UPDATE_POST_TITLE';
